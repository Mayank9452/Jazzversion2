...
import { Dot, Info, Wifi, Gem, ChevronLeft, ArrowLeft, Play } from "lucide-react";
...
function SpinWheelUpdatedVoucher3({ }) {
    ...
    const hexToRgba = (hex: string, alpha: number) => {
        const cleanHex = (hex || "#FFD600").replace("#", "");
        const r = parseInt(cleanHex.substring(0, 2), 16) || 255;
        const g = parseInt(cleanHex.substring(2, 4), 16) || 202;
        const b = parseInt(cleanHex.substring(4, 6), 16) || 32;
        return `rgba(${r}, ${g}, ${b}, ${alpha})`;
    };

    const [shadowColor, setShadowColor] = useState<string>(() => {
        if (!defaultColors || defaultColors.length === 0) return "#FFD600";
    ...
    const hasSetApiColorRef = useRef(false);

    useEffect(() => {
        if (spinWinData?.colorArray && spinWinData.colorArray.length > 0 && !hasSetApiColorRef.current) {
            const apiColors = spinWinData.colorArray;
            const randomColor = apiColors[Math.floor(Math.random() * apiColors.length)];
            setShadowColor(randomColor);
            setOutlineColor(randomColor);
            hasSetApiColorRef.current = true;
        }
    }, [spinWinData]);
    ...
    const SpinButton = ({ className = "" }) => (
        <button
            onClick={spinWheel}
            className="w-full py-2 rounded-xl flex items-center justify-center gap-1 bg-brand-gradient hover:brightness-110 text-brand-black-100 text-lg font-bold shadow-md active:scale-90 transition-all shrink-0 pointer-events-auto dark:border-2 border-white"
    ...
    return (
        <div className="bg-slate-50 dark:bg-[#191919] text-slate-900 dark:text-white transition-colors duration-300 relative overflow-hidden pb-4">
            ...
            <div className="pb-4 relative z-30">
                <div className={`relative overflow-hidden p-2 flex items-center justify-between gap-3 border-b transition-all duration-300 ${isDarkTheme ? "bg-gradient-to-br from-[#2B2B2B]/40 to-[#191919]/30 border-white/[0.06] shadow-[0_12px_40px_rgba(0,0,0,0.2)]" : "bg-gradient-to-br from-white/70 to-white/40 border-slate-200/60 shadow-sm"} backdrop-blur-xl`}>
            ...
            <div className="flex flex-col gap-0 p-4 cs-pt-0 pt-4 space-y-0 sm:space-y-0 sm:min-h-[80vh] cs-mt-0">
                ...
                <div className="flex flex-col items-center space-y-8 mb-4">
                    <div className="relative mt-0">
                        <div className="relative flex flex-col gap-2">
                            <div className="relative flex flex-col gap-0">
                                {/* Spinner */}
                                <div className="px-[50px] sm:px-[65px] pt-2 relative flex flex-col gap-8 pb-8">
                          ...
                          before:w-[150%] before:h-10 before:transform before:translate-x-[-50%] ${isDarkTheme ? "before:bg-[radial-gradient(white,white,transparent,transparent,transparent)]" : "before:bg-[radial-gradient(black,black,transparent,transparent,transparent)]"} before:rounded-full`}
                                            ...
                                            //         : `0 0 25px 35px ${hexToRgba(shadowColor, 0.2)}`
                                            // }}
                                            >
                                        ...
                                        `}
                                                ...
                                                {/* Wheel content */}
                                                <div
                                                    className="relative h-full w-full overflow-hidden rounded-full outline outline-[20px] border-[6px] border-black"
                                                    style={{ borderStyle: "inset", outlineColor: outlineColor }}
                                                >
                                                                            ...
                                                                            >
                                                                                <div className="h-[1.4rem] sm:h-6 w-full absolute flex items-center justify-center top-[45.5%] sm:top-[47%] -start-5">
                                                                                    {segment?.id === 0 ? (
                                                                                        ...
                                                                                        return (
                                                                                            <div
                                                                                                className={`${segment.text} -ms-4 flex flex-col items-center justify-start gap-1 text-center font-pop font-extrabold px-2`}
                                                                                                style={{
                                                                                                    color: "#FFFFFF",
                                                                                                    textShadow: "0px 1px 3px rgba(0,0,0,0.9), 0px 2px 6px rgba(0,0,0,0.9)",
                                                                                                    position: "absolute",
                                                                                            ...
                                                                                            >
                                                                                                <div className="flex items-baseline justify-center gap-1 leading-none font-black text-white">
                                                    ...
                                                    {/* Central static pointer - opposite orientation (points up) */}
                                                    <div className="absolute z-[60] top-[47%] start-[50%] -translate-x-1/2 -translate-y-1/2  flex items-center justify-center pointer-events-none w-20 h-20 sm:w-24 sm:h-24">
                                                        <img
                                                            src="/assets/images/iconspin2.png"
                                                            className="h-full w-full object-contain drop-shadow-[0px_4px_6px_rgba(0,0,0,0.5)]"
                                                            alt="Pointer"
                                                            loading="lazy"
                                                        />
                                                    </div>
                    ...
                    <div className="flex justify-center w-[30%] relative z-50">
                        <SpinButton />
                    </div>
                </div>
...
export default SpinWheelUpdatedVoucher3;