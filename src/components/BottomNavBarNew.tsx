            ...
            <div className="fixed bottom-0 left-1/2 transform -translate-x-1/2 w-full max-w-[480px] z-50 grid">
                {isToggle && (
                    <div className="flex justify-between">
                        <button
                            className={cn(
                                "w-16 text-[10px] px-2 py-1 gap-1 pt-2 shadow-md flex justify-center items-center rounded-t-md border-t border-x",
                                isDark
                                    ? "bg-white dark:bg-brand-black-100 text-slate-800 dark:text-white border-slate-200 dark:border-white/10"
                                    : "bg-[#ffca20] text-black border-[#dfa208]/30 font-bold"
                            )}
                        ...
                        <button
                            className={cn(
                                "w-12 shadow-md flex justify-center items-center rounded-t-md border-t border-x",
                                isDark
                                    ? "bg-white dark:bg-brand-black-100 text-slate-800 dark:text-white border-slate-200 dark:border-white/10"
                                    : "bg-[#ffca20] text-black border-[#dfa208]/30"
                            )}
                            style={{ borderBottom: "none" }}
                        ...
                        <div className="relative w-full flex items-end">
                            ...
                            <div className="absolute inset-x-0 bottom-0 -z-10 pointer-events-none">
                                ...
                                >
                                    {!isDark && (
                                        <defs>
                                            <linearGradient id="nav-brand-gradient" x1="0" y1="0" x2="1" y2="0">
                                    ...
                                    <path
                                        ...
                                        className={isDark ? "fill-white dark:fill-brand-black-100 transition-colors duration-300" : undefined}
                                        fill={!isDark ? "url(#nav-brand-gradient)" : undefined}
                                    />
                                    {/* Border Stroke Path */}
                                    <path
                                        ...
                                        fill="none"
                                        strokeWidth="2.5"
                                        className={isDark ? "stroke-slate-200/80 dark:stroke-white/10 transition-colors duration-300" : "opacity-20"}
                                        stroke={!isDark ? "#dfa208" : undefined}
                                    />
                                </svg>
                            </div>
                                    ...
                                    return (
                                        ...
                                        >
                                            {isPlayWin ? (
                                                <div
                                                    className={cn(
                                                        "absolute bottom-1 w-[80px] h-[80px] rounded-full border-4 flex items-center justify-center transition-all duration-300 overflow-hidden hover:brightness-110",
                                                        isDark
                                                            ? "bg-brand-gradient border-slate-100 dark:border-white shadow-[0_0_9px_rgba(255,202,32,0.65)]"
                                                            : "bg-gradient-to-br from-[#252525] via-[#121212] to-[#0a0a0a] border-[#dfa208] shadow-[0_4px_12px_rgba(0,0,0,0.3)]",
                                                        isActive && (isDark ? "ring-2 ring-brand-yellow-100/60 scale-105" : "ring-2 ring-white/20 scale-105")
                                                    )}
                                                >
                                                    {/* Pulsing ring background glow */}
                                                    <div className={cn(
                                                        "absolute inset-0 rounded-full pointer-events-none",
                                                        isDark ? "bg-brand-yellow-100/20 animate-ping opacity-60 scale-105" : "bg-brand-yellow-100/10 animate-pulse scale-110"
                                                    )} />
                                                    ...
                                                    <AnimatePresence mode="wait">
                                                        ...
                                                        {currentState === "text" && (
                                                            <motion.div
                                                                ...
                                                                exit={{ opacity: 0, scale: 0.6, y: -8 }}
                                                                transition={{ type: "spring", stiffness: 300, damping: 20 }}
                                                                className="flex flex-col items-center justify-center z-10 font-black leading-none select-none"
                                                            >
                                                                <span className={cn("text-sm tracking-wider font-black uppercase", isDark ? "text-white dark:text-brand-black-100" : "text-[#ffca20]")}>Play</span>
                                                                <span className={cn("text-xs tracking-widest font-black uppercase mt-0.5", isDark ? "text-white dark:text-brand-black-100" : "text-[#ffca20]")}>& Win</span>
                                                            </motion.div>
                                                        )}
                                                ...
                                                <>
                                                    ...
                                                    {isActive && (
                                                        <motion.div
                                                            ...
                                                            className={cn(
                                                                "absolute bottom-1.5 w-4 h-0.5 rounded-full shadow-[0_0_6px_rgba(0,0,0,0.5)]",
                                                                isDark ? "bg-[#ffca20] dark:bg-brand-yellow-100" : "bg-black"
                                                            )}
